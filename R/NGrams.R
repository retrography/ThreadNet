...
# Counts n-grams within threads where the length of the thread is greater than n.
# Returns a dataframe with ngram, frequency and proportion in descending order
count_ngrams <- function(o,TN,CF,n){

	# o dataframe containing threads
...
maximal_ngrams <- function(ng){
  ...
  w <- lapply( 1:nrow(ng), function(i){ grep(ng$ngrams[i],ng$ngrams) } )

  # get howMany times each one appears
  howMany <- lapply( 1:length(w), function(i){ length(w[[i]]) } )
...
support_level <- function(tv, ng) {
	...
	# change the commas back to spaces
  	...
  	ng$support <- unlist( lapply( 1:nrow(ng), function(i){ length(grep(ng$ngrams[i],tv)) } ) )/totalN
...
generativity_level<- function(tv, ng){
  	...
  	for (n in nList){
  		...
  		# pick the ngrams of length n from the list given
  	  	...
  	  	w <- lapply( 1:nrow(ngn), function(i){ grep(ngn$ngrams[i],ngplus$ngrams) } )

  	  	zplus <- lapply( 1:nrow(ngn), function(i){ str_locate(ngplus$ngrams[w[[i]]],ngn$ngrams[i]) } )
...
long_enough = function(tv,n,delimiter){ tv[ unlist(lapply(1:length(tv), function(i) {length(unlist(strsplit(tv[[i]],delimiter)))>=n})) ] }
